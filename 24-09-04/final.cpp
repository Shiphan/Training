...
public:
	...
	void setNext(std::shared_ptr<ListNode<T>> newNext) {
		this->next = newNext;
	}
	void dropAllAfter() {
...
public:
	...
	void append(const T& data) {
		if (this->head == nullptr) {
			this->head = std::make_shared<ListNode<T>>(data);
			return;
		}
		...
		ptr->setNext(std::make_shared<ListNode<T>>(data));
	}
