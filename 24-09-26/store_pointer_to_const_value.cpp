...
public:
	Stack(size_t max_stack_size) {
		...
		this->stack = new const T*[max_stack_size];
		this->top = 0;
	}
	~Stack() {
		delete[] this->stack;
...
int main() {
	...
	std::cout << "# test 1:" << std::endl;
	{
		...
		const int* out_1 = stack.pop();

		int b = 2;
		stack.push(b);

		const int* out_2 = stack.pop();
	...
	std::cout << "# test 3:" << std::endl;
	{
		...
		const Node* out_1 = stack.pop();

		Node b(2);
		stack.push(b);

		const Node* out_2 = stack.pop();
	...
	std::cout << "this test not working as pop return a pointer to const T" << std::endl;
	{
		Stack<int> stack(10);
